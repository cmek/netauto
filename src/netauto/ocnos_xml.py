...
def extract_rpc_error_info(xml_string):
    ...
    error_info = {}
    for tag in ['error-type', 'error-tag', 'error-severity', 'error-app-tag', 'error-path', 'error-message']:
...
def build_interface_config(interface: Interface) -> str:
    ...
    interfaces = ET.SubElement(config, "interfaces", xmlns="http://www.ipinfusion.com/yang/ocnos/ipi-interface")
...
def build_lag_config(lag_number: int, members: List[str], lacp_mode: str = "active", min_links: int = 1, mtu: int = 1500) -> str:
    """
...
# member interfaces
    ...
    for member in members:
        ...
        member_agg = ET.SubElement(mem_intf, "member-aggregation", xmlns="http://www.ipinfusion.com/yang/ocnos/ipi-if-aggregate")
...
def build_lag_delete(name: str, members: List[str]) -> str:
    ...
    lag_intf = ET.SubElement(config, "interface", xmlns="http://www.ipinfusion.com/yang/ocnos/ipi-interface", operation="delete")
    ET.SubElement(lag_intf, "name").text = name
    
    # Unbind members (optional depending on device behavior, but safer to remove config)
    for member in members:
        mem_intf = ET.SubElement(config, "interface", xmlns="http://www.ipinfusion.com/yang/ocnos/ipi-interface")
        ET.SubElement(mem_intf, "name").text = member
...
def build_evpn_service(vlan_id: int, vni: int, vrf_name: str, rd: str, rt_import: List[str], rt_export: List[str], s_tag: Optional[int] = None) -> str:
    """
    ...
    # VLAN
    vlan_db = ET.SubElement(config, "vlan-database", xmlns="http://www.ipinfusion.com/yang/ocnos/ipi-vlan")
    ...
    # VRF
    vrf = ET.SubElement(config, "vrf", xmlns="http://www.ipinfusion.com/yang/ocnos/ipi-vrf")
    ...
    # VXLAN
    vxlan = ET.SubElement(config, "vxlan", xmlns="http://www.ipinfusion.com/yang/ocnos/ipi-vxlan")
...
def build_evpn_delete(vlan_id: int, vrf_name: str) -> str:
    ...
    vrf = ET.SubElement(config, "vrf", xmlns="http://www.ipinfusion.com/yang/ocnos/ipi-vrf", operation="delete")
    ET.SubElement(vrf, "name").text = vrf_name
    
    # Delete VXLAN mapping
    vxlan = ET.SubElement(config, "vxlan", xmlns="http://www.ipinfusion.com/yang/ocnos/ipi-vxlan", operation="delete")
    ET.SubElement(vxlan, "vlan").text = str(vlan_id)
